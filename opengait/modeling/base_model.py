...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        self.split_mimic = cfgs['data_cfg']['split_mimic']
        ...
        #if not training:
        if 'eval_multiple_times' in self.engine_cfg.keys():
            self.eval_multiple_times = self.engine_cfg['eval_multiple_times']
        else:
            self.eval_multiple_times = False
    ...
    def log_results_wandb(model, result_dict):
        #result_dict is a dict of [str: list]. str -> distance, list -> accuracy at different ranks
        #Logs rank 1 and rank 20 accuracies of different ranges
        
        # results_to_log = {}
        # for k,v in result_dict.items():
        #     results_to_log[f"{k}_1"] = v[0]
        #     results_to_log[f"{k}_20"] = v[19]
        
        results_to_log = result_dict
        if torch.distributed.get_rank() == 0:
            print(results_to_log)
    ...
    def run_train(model):
        ...
        # if torch.distributed.get_rank() == 0:
        #     print(f"Test results: {result_dict}")
        ...
        for inputs in model.train_loader:
            ...
            print(f"Training iterations started") if model.iteration == 1 and torch.distributed.get_rank() == 0 else None
            
            # if torch.distributed.get_rank() == 0 and model.iteration == 10:
            #     torch.autograd.set_detect_anomaly(True)
            
            if torch.distributed.get_rank() == 0 and model.cfgs['trainer_cfg']['wandb']:
            ...
            if model.iteration % model.engine_cfg['save_iter'] == 0:
                ...
                if model.engine_cfg['with_test']:
                    ...
                    model.train()
                    if torch.distributed.get_rank() == 0 and model.cfgs['trainer_cfg']['wandb']:
                        model.log_results_wandb(result_dict)
    ...
    def run_test(model):
        ...
        if model.eval_multiple_times:
            import pickle
            print(f"Running test 10 times!")
            print("WARNING: This should only be used with phase = test.")
            ...
            for i in range(10):
                ...
                if rank == 0:
                    ...
                    result = eval_func(info_dict, dataset_name, **valid_args)
                    print(f"Evaluation {i} complete!")
                    print(result)
                    result_list.append(result)
            
            pkl_name = f'{model.engine_cfg["save_name"]}_eval_10times.pkl'
            with open(pkl_name, 'wb') as f:
                pickle.dump(result_list, f)
            print(f"Saved results to {pkl_name}")
            return result       # Return the last result to be consistent with the original code
