...
class GaitGL_occ_aware(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        if dataset_name in ['OUMVLP', 'GREW', 'Gait3D']:
            # For OUMVLP and GREW
            ...
            raise NotImplementedError("This model is only implemented for OUMVLP, GREW and Gait3D datasets!")
        ...
        self.occ_mixer_fc = SeparateFCs(**model_cfg['OccMixerFCs'])     #(parts_num, original_gaitgl embed size, desired size of occlusion aware embedding)
        ...
        if 'SeparateBNNecks' in model_cfg.keys():
            ...
            self.Head1 = SeparateFCs(64, in_c[-1], class_num)         #(64+in_c because desired occlusion mixed size is twice of gaitgl normal embed size)
            self.Bn_head = True
    ...
    def forward(self, inputs):
        ...
        #pdb.set_trace()
        with torch.no_grad():
            occ_embed = self.occ_detector(sils, seqL)     #(Batch, embed)
