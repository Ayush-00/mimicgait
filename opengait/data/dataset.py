...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        ...
        # self.split_mimic = data_cfg['split_mimic']
        # if self.split_mimic:
        #     self.occlusion_range = data_cfg['occlusion_range']  # % Range of frame height/width to split 
    ...
    def __loader__(self, paths):
        ...
        for pth in paths:
            ...
            # if self.split_mimic:
            #     # Split into visible and invisible
            #     visible, invisible = self.split_visible_invisible(_)
            #     data_list.append(visible)
            #     data_list.append(invisible)
            # else: 
            #     data_list.append(_)
            data_list.append(_)
                ...
                for typ in sorted(os.listdir(osp.join(dataset_root, lab))):
                    for vie in sorted(os.listdir(osp.join(dataset_root, lab, typ))):
                        ...
                        if seq_dirs != []:
                            ...
                            new_seq_dirs = []
                            for dir in seq_dirs:
                                if self.dataset_name == 'Gait3D':
                                    if dir.endswith('img_64.pkl'):
                                        new_seq_dirs.append(dir)
                                else: 
                                    new_seq_dirs.append(dir)
